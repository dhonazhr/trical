...
with st.sidebar:
    selected=option_menu(
        menu_title="Menu",
        options=["Beranda","Pengertian","Kalkulator","Tentang Sistem"],
        icons=["bank","book","calculator-fill","bell-fill"],
        menu_icon="cast",
        default_index=0,
        )
...
if selected=="Kalkulator":
     st.title('''
    :violet[Selamat Datang di Web trical]''')
     kalkulator=option_menu(
         menu_title=None,
        options=["Normalitas","%(b/b)","%(b/v)","BE"],
        icons=["calculator-fill","calculator-fill","calculator-fill","calculator-fill"],
        menu_icon="cast",
        default_index=0,
    ...
    #%(b/b)
     if kalkulator=="%(b/b)":
         ...
         BE = st.number_input('Masukkan nilai BE (mg/mgrek)',format="%.1f",key='BE')
         FP = st.number_input('Masukkan nilai FP',format="%.0f",key='FP')
         gram = st.number_input('Masukkan nilai gram (g)',format="%.4f",key='gram')
         
         tombol1 = st.button('Hitung nilai kadarnya',key='tombol1')
         
         if tombol1:
             nilai_kadar = (Vsampel*Ntitran2*BE*10**-3*FP*100)/gram 
    ...
    #%(b/v)
     if kalkulator=="%(b/v)":
         ...
         if tombol2:
             nilai_kadar = (Vtitran*Ntitran*BE*10**-3*FP*100)/Vtitrat 
             st.success(f'Persentase kadarnya adalah {nilai_kadar:.2f}%(b/v)')
